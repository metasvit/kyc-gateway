...
function hashLeaf([address, data]) {
...
class ShardedMerkleTree {
  ...
  getProof(address) {
    ...
    const data = shard.entries[address];
    const leaf = hashLeaf([address, data]);
    const proof = this.trees[shardid]
      .getProof(leaf)
      .map((entry) => "0x" + entry.data.toString("hex"));
    return [data, proof.concat(shard.proof)];
  }
  ...
  static build(entries, shardNybbles, directory) {
    ...
    for (const [address, data] of entries) {
      ...
      shards[shard].push([address, data]);
  ...
  static fromFiles(directory) {
    const fs = require("fs");
    const path = require("path");
    const { root, shardNybbles, total } = JSON.parse(
